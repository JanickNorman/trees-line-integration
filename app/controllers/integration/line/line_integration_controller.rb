class Integration::Line::LineIntegrationController < ApplicationController
  ...
  def admin
    ...
    logger.debug integration_params.to_json
  ...
  def channelback
    ...
    render json: {external_id: integration_params[:request_unique_identifier]}, status: :ok
  end
  ...
  def line_webhook
    logger.info "Tes logging from Norman"
    logger.info params.to_yaml
    logger.info integration_params.to_yaml
    ...
    if events[:type] == "message"
      ...
      # logger.info @channel.to_yaml
      # logger.info integration_params.to_yaml
      # logger.info "This is from channel"
      # logger.info @channel.to_yaml
      if request.headers["X-Line-Signature"] == signature
        ...
        if @user
          ...
          # if author's last created ticket is updated x hours ago, create a new ticket, else comment that latest ticket



          logger.debug payload.to_json
          #response = RestClient.post {"https://#{@channel.zendesk_subdomain}.zendesk.com/api/v2/tickets", payload, {content_type: 'application/json', accept: "application/json", authorization: "Bearer #{@channel.zendesk_access_token}"}, payload
          response = RestClient.post "https://#{@channel.zendesk_subdomain}.zendesk.com/api/v2/any_channel/push", payload.to_json, {content_type: 'application/json', accept: "application/json", authorization: "Bearer #{@channel.zendesk_access_token}"}

          #put external resource to database
          logger.debug "INIRESPONSE"
          response_body = JSON.parse(response)
          logger.debug response_body["results"].first
          logger.debug response_body["results"].first["status"]["code"]

          # begin
          #
          #   if response[:results].first[:status][:code] == "success"
          #     #author_last_resource = ExternalResource.find(external_id: 123456)
          #     ext_resource = ExternalResource.initialize(external_id: events[:message][:id])
          #     ext_resource.message = events[:message][:text]
          #     ext_resource.created_at = events[:timestamp]
          #     ext_resource.line_timestamp = events[:timestamp]
          #     ext_resource.author_id = events[:source][:userId]
          #     ext_resuorce.allow_channelback = true
          #     ext_resuorce.updated_at = events[:timestamp]
          #     ext_resuorce.save
          #   else
          #
          #   end
      ...
      external_resources: [
        {
          ...
          author: {
            ...
            name: @user.name,
            image_url: @user.image_url || "http://www.makemac.com/wp-content/uploads/2014/07/Line-Logo.jpg"
          },
          allow_channelback: true
        }
      ]
  ...
  def send_reply_url
    ...
    channel = Channel.find_or_initialize_by(channel_id: @line_channel_id)
    ...
    # end
    channel.name = @name
    channel.zendesk_subdomain = @subdomain
    channel.channel_id = @metadata['line_channel_id']
    channel.channel_secret = @metadata['line_channel_secret']
    channel.channel_access_token = @metadata['line_channel_access_token']
    channel.zendesk_locale = @locale
    ...
    channel.zendesk_access_token = @zendesk_access_token
    channel.save
    logger.debug "This is the new channel"
